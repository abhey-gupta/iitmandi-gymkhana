...
const ManageRecommendationRequestsSociety = () => {
  ...
  const role = Cookies.get("society_member_role");

  useEffect(() => {
    (async () => {
      const { data } = await axios.post("/api/society-member/fetch-by-token", {
        society_member_token: Cookies.get("society_member_token"),
      });
      if (data.success) {
        console.log(data.society_member);
        setSocietyMember(data.society_member);
      }
    })();
  }, []);
  ...
  const handleAddComment = async () => {
    try {
      const { data } = await axios.post("/api/request/add-comment", {
        member_token: Cookies.get("society_member_token"),
        comment,
        id: selectedRequest,
      });
      console.log(data);
  ...
  const approveRequest = async (id) => {
    try {
      ...
      if (data.success) {
        console.log("here");
        const updatedRequest = data.request;
        await axios.post("/api/club/update-expense", {
          member_token: Cookies.get("society_member_token"),
          amount: updatedRequest.total_amount,
        });
        const res = await axios.post("/api/society/update-expense", {
          member_token: Cookies.get("society_member_token"),
          amount: updatedRequest.total_amount,
        });
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await axios.post("/api/request/fetch-all");
      ...
      <Table>
        ...
        <TableBody>
          {requests.map((request) => {
            // console.log(request.recommendedBy);
            return (
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            console.log(societyMember.monthly_expense);

                            if (
                              societyMember.monthly_expense &&
                              Object.keys(
                                societyMember.monthly_expense
                              ).includes(formattedDate)
                            ) {
                              if (
                                societyMember.monthly_expense[formattedDate] +
                              ...
                              if (parseFloat(request.total_amount) < 30000) {
                                approveRequest(request._id);
...
export default ManageRecommendationRequestsSociety;
